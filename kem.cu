...
#elif(FPTRU_BOUND == 5) //761和1277
    poly_sample_and_double_v2<<<BATCH_SIZE,FPTRU_N / 8 + 1, 0, stream_0>>>(f_0,coins_d_0,1,1);
    poly_sample_and_double_v2<<<BATCH_SIZE,FPTRU_N / 8 + 1, 0, stream_0>>>(g_0,coins_d_0 + FPTRU_COIN_BYTES / 2,0,0);
...
#if(NEW == 1)
    BEFORE_SPEED
    pack_sk_batch_v2<<<BATCH_SIZE,FPTRU_N/8 + 1,0,stream_0>>>(sk_d_0,f_0);
...
#elif(FPTRU_BOUND == 5) //761和1277
    poly_sample_and_double_v2<<<BATCH_SIZE,FPTRU_N / 8 + 1, 0, stream>>>(f_0,coins_d_0,1,1);
    poly_sample_and_double_v2<<<BATCH_SIZE,FPTRU_N / 8 + 1, 0, stream>>>(g_0,coins_d_0 + FPTRU_COIN_BYTES / 2,0,0);
...
double crypto_kem_keygen_no_inv_batch_stream_ordered_async(unsigned char *array_pk,unsigned char *array_sk,cudaStream_t stream){//unsigned char *pk,unsigned char *sk){
    ...
    //BEFORE_SPEED
    //poly_inverse_batch_0_v3(finv_0,f_0,stream);
    poly_inv<<<BATCH_SIZE,FPTRU_N ,0,stream>>>(finv_0,f_0); //TODO:修改这里的线程组织形式
    //AFTER_SPEED("v2")
...
void crypto_kem_keygen_tensor_core(unsigned char *array_pk,unsigned char *array_sk,cudaStream_t stream){//unsigned char *pk,unsigned char *sk){
    ...
    //BEFORE_SPEED
    //poly_inverse_batch_0_v3(finv_0,f_0,stream);
    poly_inv<<<BATCH_SIZE,FPTRU_N + 1,0,stream>>>(finv_0,f_0);
    //AFTER_SPEED("v2")
    ...
    tensor_poly_mul(h_0,finv_0,g_0,stream);
...
double crypto_kem_encaps_v4(unsigned char *ct, unsigned char *k, const unsigned char *pk,cudaStream_t stream){
    ...
    HANDLE_ERROR(cudaMallocAsync((void**)&ct_d, FPTRU_PKE_CIPHERTEXTBYTES * BATCH_SIZE,stream));
    poly_fqcsubq_encode_compress_batch_pack_ct<<<BATCH_SIZE,FPTRU_N/8 + 1,0,stream>>>(ct_d,sigma_h_d,m_d+FPTRU_PREFIXHASHBYTES,(FPTRU_PREFIXHASHBYTES + FPTRU_MSGBYTES));
